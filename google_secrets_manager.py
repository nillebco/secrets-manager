...
class GoogleSecretsManager(SecretsManager):
    ...
    def __init__(self, project_id: Optional[str] = None):
        ...
        if not self.project_id:
            raise RuntimeError(
                "No project ID provided and no default project set in gcloud config"
            )
    ...
    def store_secret(
        ...
        try:
            ...
            try:
                subprocess.run(
                    ["gcloud", "secrets", "create", name, "--project", self.project_id],
                    check=True,
                    capture_output=True,
                )
            except subprocess.CalledProcessError:
    ...
    def list_secrets(self, project_id: Optional[str] = None) -> list[Dict[str, Any]]:
        ...
        try:
            ...
            target_project = project_id or self.project_id
            if not project_id:
                logging.warning(f"Using default project: {self.project_id}")
    ...
    def list_organizations(self) -> list[Organization]:
        ...
        print(organizations)
        return [
            Organization(id=org["name"], name=org["displayName"])
